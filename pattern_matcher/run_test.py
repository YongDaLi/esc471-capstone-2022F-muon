...
decayWindow = 5
S1ToS2Window = (
    5  # used for a time window for muon to travel between any two horizontal levels
)
...
class muon:
    def __init__(self):
        ...
        if self.DecayedInS1:
            ...
            if self.EnteredS2:
                ...
                if self.DecayedInS2:
                    self.timestamps[2].append(
                        self.S2EntryTime + rd.randint(1, decayWindow)
                    )
                else:
                    # muon has reached Al plate
                    if self.DecayedInAl:
                        if self.ElectronUp:
                            self.ElectronS2EntryTime = (
                                self.S2EntryTime
                                + rd.randint(1, decayWindow)
                                + rd.randint(1, S1ToS2Window)
                            )
                            self.timestamps[2].append(self.ElectronS2EntryTime)
                        elif not self.ElectronUp:
                            self.ElectronS3EntryTime = (
                                self.S2EntryTime
                                + rd.randint(1, decayWindow)
                                + rd.randint(1, S1ToS2Window)
                            )
    ...
    def getTimestamps(self):
        return (
            np.array(self.timestamps[1]),
            np.array(self.timestamps[2]),
            np.array(self.timestamps[3]),
        )
