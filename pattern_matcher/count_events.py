...
S1ToS2Window = (
    5  # time needed for a muon to travel from scintillator 1 to scintillator 2
)
...
def decayedDown(S2, S3, t):
    ...
    for i in range(len(S3)):
        if S3[i] < S2[t]:
            continue
        if (S3[i] - S2[t]) < S2ToElectronPeakWindow and (
            S3[i] - S2[t]
        ) > decayWindow / 3:  # note that decayWindow is a upper threshold, but there we want a lower threshold. Use /3 as extimate.
            return True
    return False
...
def decayedUp(S2, t):
    ...
    for i in range(t + 1, len(S2)):
        if (S2[i] - S2[t]) < S2ToElectronPeakWindow and (
            S2[i] - S2[t]
        ) > decayWindow / 3:
            return True
    return False
...
def countEvents(S1, S2, S3):
    ...
    for i in range(0, len(S1), 1):
        ...
        if decayedDown(S2, S3, S2EntryPeak[0]):
            ...
            S2, S2EntryPeak[0]
        ):  # elif so don't double count a decay in metal plate for both down and up
            log(S1[i])
